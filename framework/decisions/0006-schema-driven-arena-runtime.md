...
<!-- Bootstrap note: in the reference project this record also lists the
     per-module rule keys it covers (exec.soa, exec.zero_alloc, exec.reentrant,
     exec.declared_layout). This copy ships with no `rules:` entries because a
     fresh project has no intents yet and the CLI fails records that reference
     unknown keys. When your first intent declares those exec.* rules, either
     add them to this record's `rules:` list or cover them in your own
     feature-scoped records. -->

## Context

The first version of the reference exec unit computed nine byte offsets by hand.
...
fields. A 13-field arena is fine either way; a real domain arena with 20+ parallel
field arrays is not — the reference project's second module crossed the threshold
with 26 handle properties and its suite asserts fast properties directly.
...
- `new Function` is unavailable under a strict CSP without `unsafe-eval`. This does
  not affect Node, but it constrains a browser build.
- Layout order no longer matches declaration order. Reading raw bytes requires
  `singularity layout <workflow>` rather than reading the source top to bottom.
...
- **Hand-written offsets per module.** Correct once, then duplicated and wrong.
