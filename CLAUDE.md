...
`framework/` is the copy-to-bootstrap kit for new projects. Its mechanical core
(CLI, runtime, ruleset, lint, arena suite) must stay byte-identical to the live
files — `tests/framework-sync.assert.js` fails `check` if it drifts. When you
change any of those live files, re-copy them into `framework/`.
